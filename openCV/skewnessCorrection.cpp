 ...
 * 
 * Fourier transform is used to find the skewness in the image.
 */
...
{
    CommandLineParser parser(argc, argv, "{@input | ./img/text.jpg | input image}");

    // Load image 
    Mat image = imread( parser.get<String>("@input"), IMREAD_COLOR);
    if (image.empty())
    {
    ...
    //Show the image
    imshow("Input", image);
    imwrite("deskewed_image.jpg", rotated);
    waitKey(0);
    return 0;
}